...
import { getConfigPaths, getProjectRoot } from '../utils/paths.js';
...
// 默认官方系统提示词（从外部文件读取）
const systemInstructionPath = path.join(getProjectRoot(), 'system_instruction.txt');
let DEFAULT_OFFICIAL_SYSTEM_PROMPT = '';
try {
  if (fs.existsSync(systemInstructionPath)) {
    DEFAULT_OFFICIAL_SYSTEM_PROMPT = fs.readFileSync(systemInstructionPath, 'utf-8').trim();
    log.info(`✅ 已从外部文件加载官方系统提示词 (${DEFAULT_OFFICIAL_SYSTEM_PROMPT.length} 字符)`);
  }
} catch (e) {
  log.error(`❌ 加载外部系统提示词失败: ${e.message}`);
}
...
function getActiveApiConfig(jsonConfig) {
  ...
  return {
    ...
    userAgent: `antigravity/${jsonConfig.api?.version || "1.20.5" } windows/amd64`,
    ideVersion: jsonConfig.api?.version || "1.20.5",
    unleash: unleash
  };
}
...
export function buildConfig(jsonConfig) {
  ...
  return {
    ...
    // 完美复刻官方协议包 (默认开启)
    perfectProtocol: jsonConfig.other?.perfectProtocol !== false,
