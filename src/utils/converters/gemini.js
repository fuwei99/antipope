...
import { wrapOfficialProtocol, getOfficialTools } from './officialProtocol.js';
...
export function generateGeminiRequestBody(geminiBody, modelName, token) {
  ...
  // 完美复刻工具注入 (排除图片生成模型)
  const isImageModelForTools = actualModelName.includes('-image');
  if (config.perfectProtocol && !isImageModelForTools) {
